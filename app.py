...
import datetime
# pip3 install flask flask-cors flask-sqlalchemy sqlalchemy flask-marshmallow marshmallow marshmallow-sqlalchemy mysql-connector-python


# To turn on virtual environment for mac
# python3 -m venv venv
...
app.config['SQLALCHEMY_DATABASE_URI'] = "mysql+mysqlconnector://root:sqlpass#4@localhost/e_commerce_db2"
...
def get_customer(customer_id): 
    query = select(Customer).filter_by(customer_id=customer_id)
...
@app.route('/customer_account/<id>', methods=['GET'])
def get_customer_account(id):
    account = CustomerAccount.query.get(id)
    return account_schema.jsonify(account)

@app.route('/customer_account/<id>', methods=['PUT'])
...
@app.route('/customer_account/<id>', methods=['DELETE'])
